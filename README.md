...
#### **方法1: 実行ファイル版（推奨）**
1. **ダウンロード**
   - [Releases]([リンク]https://github.com/eito54/Gemisoku-GUI/releases/tag/1.0.0)から最新版の`Gemisoku-GUI-Setup.exe`をダウンロード
...
#### **3. OBSソース設定**
1. **MK8DX画面をキャプチャ**
   - OBSで「映像キャプチャデバイス」または「ウィンドウキャプチャ」を追加
   - Nintendo Switchの画面が映ることを確認

2. **ソース名を確認**
   - OBSのソース一覧で表示される名前をGemisoku-GUIの設定に入力
...
1. **オーバーレイURLを取得**
   ...
   - URLをコピー: `http://localhost:3000/static?overlay=true`
