...
- 更新日: 2026-08-24
- 基準コミット: v4.0.0 リリース済み（タグ・mainともにpush済み）

## 1. リリース済み

| 版 | 状態 |
|---|---|
| **v4.0.0** ✅ 最新 | GitHub Releases公開済み。24人スタンドモード/dc対策/校正UI/Qwen3.6切替/完全バイリンガル/デザイン刷新を含む |
| v3.2.0 | 公開済み（自動更新により v4.0.0 への更新が配信中） |

## 2. v4.0.0 内容サマリ

- **24人スタンド読取モード**: 列ごと校正UI（プレビュー付き）+ 2画像OCR + 上書き集計
- **dc対策**: 減少検出→確認→オフセット永続化で自動復元
- **変化ハイライト / 欠席プレイヤー保持**
- **解析モデル**: Qwen 3.6 27B（reasoning_effort none）
- **デザイン刷新**: レーシングアクセント配色・ネストスクロール解消・コントラスト改善・絵文字撤去・トークン化
- **完全バイリンガル**: 使用中t()キー279個、ロケール差分ゼロ（`scripts/check-i18n-keys.cjs`で検証）

詳細な経緯は `docs/superpowers/specs/` 配下の各設計書を参照。

## 3. 既知の残課題

| # | 内容 | 優先度 |
|---|---|---|
| R-1 | 24人モードの実機検証（校正精度・dc対策・回帰） | 高 |
| R-2 | App.tsx約3,300行の分割 / ESLint・CI整備 / 単体テスト | 中 |
| R-3 | mainプロセス側日本語メッセージはi18n対象外 | 低 |
| R-4 | フォント: カスタム書体は不採用（ユーザー好み）。再挑戦時は要事前承認（DESIGN.md記載） | — |

## 4. 検証ツール

- `node scripts/check-jp-strings.cjs` — ユーザー表示日本語の残存検出
- `node scripts/check-i18n-keys.cjs` — t()キーとロケール定義の差分チェック
- `npx electron scripts/design-capture.cjs` — 各タブのスクリーンショット取得
- `npx electron scripts/design-audit.cjs` — DOMメトリクス監査

## 5. 次のアクション候補

1. v4.0.0 の実機検証（特にスタンド読取精度）
2. 検証フィードバック次第: 校正調整 or ホットフィックス
3. App.tsx分割などの技術的負債解消
...
| **F** | App.tsx残存スイープ | 🔄 **部分完了** — アップデートUI・dash追加分は反映済み（`4e9f607`）。**残り約70行**: マッピング画面／オーバーレイ設定タブラベル群／リオープン画面ボタン類／システム設定タブ／スコア表ヘッダーと編集ボタン |

検証ツール:
- `node scripts/check-jp-strings.cjs` — コメント除外のうえユーザー表示日本語の残存行を列挙（現在76行、うちコメント5行・既定値例外1件）
- `node scripts/check-i18n-keys.cjs` — 使用中t()キーとロケール定義の差分チェック（現在 missing 0）
